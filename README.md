...
## INTRODUCCIÓN
Este proyecto consiste en una aplicación desarrollada en java que simula el funcionamiento de un mercado
de valores, en el cual se pueden realizar distintas operaciones, en función del tipo de usuario que seas.
Todo este sistema tiene una base de datos de respaldo donde se almacenan todas las transacciones que se 
van realizando. 

En este repositorio se incluye la carpeta con las clases de java necesarias para utilizar la interfaz (`src`), una carpeta con los scripts necesarios para la creación de la base de datos (`scriptsSQL`) así como el archivo de propiedades de la base de datos utilizada (`baseDatos.properties`). Por último, se incluyen también los archivos `postgresql.jar` y `jcalendar-1.4.jar` necesarios para establecer nuestra conexión usando JDBC y mostrar el calendario en la interfaz gráfica.

## COLABORADORES

Este proyecto ha sido realizado por los alumnos de la Universidad de Santiago de Compostela Nicolás Vilela Pérez, Pablo Souto Soneira, Carlos Rozas Rodríguez, Pablo Ubeira Carramal, Adrián Vidal Lorenzo y yo, Carlos Rodríguez Fernández.
